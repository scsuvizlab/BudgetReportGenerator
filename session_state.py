"""
Session State Management with LLM Integration - FIXED VERSION

Fixed to prevent automatic LLM analysis that blocks the GUI.
"""
...
class SessionState:
    ...
    def initialize_llm(self, api_key: str, cost_limit: float = None) -> bool:
        ...
        try:
            ...
            if success:
                ...
                # FIXED: Don't automatically analyze template - make it manual only
                # This was causing the GUI to freeze when enabling LLM
                # Users can manually trigger analysis using the "Analyze with LLM" button
    ...
    def load_template(self, template_path: Path) -> bool:
        ...
        try:
            ...
            # FIXED: Don't automatically analyze template with LLM
            # This was causing the GUI to freeze when templates were loaded after LLM was enabled
            # Users can manually trigger analysis using the "Analyze with LLM" button
    ...
    def load_budget(self, budget_path: Path) -> bool:
        ...
        try:
            ...
            # Try to auto-match existing mappings using basic heuristics only
            # Don't automatically trigger LLM analysis
            if self.template:
                self._auto_match_fields()  # Use basic heuristic matching only
    ...
    def enhanced_auto_match_fields(self) -> None:
        ...
        if self.llm_enabled:
            try:
                # FIXED: Added manual template analysis step first
                # Only analyze template if we haven't already
                if not self.field_suggestions and self.template:
                    try:
                        logger.info("Starting LLM template analysis...")
                    except:
                        pass
                ...
                try:
                    logger.info("Starting enhanced field matching...")
                except:
                    pass
    ...
    def _auto_match_fields(self) -> None:
        ...
        for field_name, mapping in self.mappings.items():
            ...
            if matches:
                ...
                # Safe logging to avoid recursion
                try:
                    logger.info(f"Auto-matched field to ${best_match.value:,.2f}")
                except:
                    pass
            else:
                # Safe logging to avoid recursion
                try:
                    logger.warning("No match found for field")
                except:
                    pass
    ...
    def set_manual_value(self, field_name: str, value: float, notes: str = "") -> bool:
        ...
        if field_name not in self.mappings:
            try:
                logger.error("Field not found in mappings")
            except:
                pass
            return False
        ...
        # Safe logging to avoid recursion
        try:
            logger.info(f"Manual value set for field: ${value:,.2f}")
        except:
            pass  # Ignore logging errors to prevent crashes
    ...
    def clear_manual_value(self, field_name: str) -> bool:
        ...
        if hasattr(self, 'enhanced_mappings') and field_name in self.enhanced_mappings:
        ...
        # Safe logging to avoid recursion
        try:
            logger.info("Manual value cleared for field")
        except:
            pass  # Ignore logging errors to prevent crashes
    ...
    def get_llm_usage_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # FIXED: Ensure all expected keys are present with default values
            usage.setdefault('enabled', True)
            usage.setdefault('total_cost', 0.0)
            usage.setdefault('budget_remaining', 0.0)
            usage.setdefault('budget_utilization', 0.0)
            usage.setdefault('total_operations', 0)
            usage.setdefault('llm_tokens', 0)
